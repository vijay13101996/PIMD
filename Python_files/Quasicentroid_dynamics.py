...
def QC_MF_calculation(dpotential,N,n_sample):
    ...
    for j in range(len(Q)):
        ...
        Langevin_thermostat.qcmd_thermalize(1,swarmobject,QC_q,QC_p,lambda_curr,dpotential,deltat,1000,rng)
        
        for i in range(n_sample):
            QC_MF[j] += np.mean(Velocity_verlet.dpotential_qcentroid(QC_q,swarmobject,dpotential),axis=0)
            Langevin_thermostat.qcmd_thermalize(1,swarmobject,QC_q,QC_p,lambda_curr,dpotential,deltat,500,rng)
...
def QCMD_instance(beads,dpotential,beta):
    ...
    f = open("/home/vgs23/Pickle_files/QCMD_Force_B_{}_NB_{}_N_{}_nsample_{}_deltat_0.5_NQ_20.dat".format(MD_System.beta,MD_System.n_beads,N,n_sample),'rb')       
    data = np.loadtxt(f)
    Q = data[:,0]
    QCMD_force = data[:,1]
